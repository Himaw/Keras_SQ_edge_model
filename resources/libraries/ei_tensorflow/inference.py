...
def run_segmentation_inference(interpreter: Interpreter, item: np.ndarray, specific_input_shape: 'list[int]',
    ...
    return {
        ...
        'recall': _recall,
        'debugInfoJson': '{}',  # TOOD(mat) can remove completely? check API
...
def classify_keras(input_x_file, input_y_file, mode: ClassificationMode, output_file, dir_path,
    ...
    if mode == 'object-detection':
        ...
        y_true = np.load(input_y_file, mmap_mode='r')
        y_true = y_true[:,0]  # just strip first column
    ...
    try:
        ...
        if mode == 'regression':
            ...
            if objdet_details.last_layer == 'fomo':
                y_true_labels, y_pred_labels = [], []
                for i, yp in enumerate(pred_y):
                    yt = yp.pop('y_true_labels')
                    height_width = infer_square_shape(yt)
                    output_shape = height_width, height_width
                    y_true_labels.append(np.array(yt).reshape(output_shape))
                    y_pred_labels.append(np.array(yp.pop('y_pred_labels')).reshape(output_shape))
                num_classes_including_background = num_classes + 1
                metrics = calculate_fomo_metrics(
                    np.stack(y_true_labels), np.stack(y_pred_labels),
                    num_classes_including_background)
                metrics['class_names'] = ['_background'] + class_names_training
                ...
                metrics = calculate_object_detection_metrics(
                    y_true_bbls, y_pred_bbls, num_classes
                )
                metrics['coco_metrics'] = calculate_coco_metrics(
                        y_true_bbls, y_pred_bbls, width, height, num_classes
                        )
                metrics['class_names'] = class_names_training
    ...
    if metrics is not None:
        ...
        # TODO this currently always run with float32, we want to do this
        #      with model_type as is done in post training modeling profiling
        metrics_json.set('test', 'float32', metrics)
        metrics_json.flush()
